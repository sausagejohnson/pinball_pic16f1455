...
void SYSTEM_Initialize( SYSTEM_STATE state )
{
    switch(state)
    {
        case SYSTEM_STATE_USB_START:
                ...
                OSCCON = 0xFC;  //HFINTOSC @ 16MHz, 3X PLL, PLL enabled, internal clock selected via FOSC in config bits
                ACTCON = 0x90;  //Active clock tuning enabled for USB
            #endif

                            
            LED_Enable(LED_LAUNCH_BUTTON);
            LED_Enable(LED_PLAYER_BUTTON);
            
            BUTTON_Enable(BUTTON_LEFT_FLIPPER);
            BUTTON_Enable(BUTTON_RIGHT_FLIPPER);

            BUTTON_Enable(BUTTON_PLAYER);
            BUTTON_Enable(BUTTON_LAUNCH);
            BUTTON_Enable(BUTTON_ADMIN);

            BUTTON_Enable(SWITCH_LEFT_TILT);
            BUTTON_Enable(SWITCH_RIGHT_TILT);
